...
import { readFixedString, writeFixedString } from "../src/string";
  ...
  describe("UTF8", () => {
    describe("decode", () => {
      for (const t of DATA) {
        it(`U+${t.codePoint.toString(16).toUpperCase()}`, () => {
          ...
          expect(readFixedString(a, t.encoded.length)).toBe(t.decoded);
        });
      }
    });
    ...
    describe("encode", () => {
      for (const t of DATA) {
        it(`U+${t.codePoint.toString(16).toUpperCase()}`, () => {
          const w = new Writer();
          writeFixedString(w, t.decoded, t.encoded.length);
